...
@testset "candidates" begin
    ...
    @test isempty(candidates(XiaoGimbutas, Simplex{2}(), PolynomialDegree(21)))
...
@testset "available ranks by node count (exit criterion)" begin
    ...
    @test length(a0) >= 3
    @test any(row -> row.family == "XiaoGimbutas" && row.degrees == 0:20, a0)
...
@testset "rule selects the best candidate and records why" begin
    ...
    @test family(rule(Simplex{2}(); degree = 25)) == "ConicalProduct"      # beyond the seeded range
    @test family(rule(Simplex{2}(); degree = 5, T = Rational{BigInt})) == "GrundmannMöller"
    @test family(rule(Simplex{3}(); degree = 5)) in ("GrundmannMöller", "ConicalProduct")
...
@testset "diagnostics" begin
    ...
    @test occursin("XiaoGimbutas", msg) && occursin("0–20", msg)
    @test occursin("0–∞", msg)
    e2 = try
        rule(XiaoGimbutas(), Simplex{2}(); degree = 30)
    catch err
        err
    end
    @test e2 isa NoRuleError
    @test occursin("Nearest: XiaoGimbutas degree 20 (79 points)", sprint(showerror, e2))
    e3 = try
