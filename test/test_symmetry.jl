...
@testset "S₄ invariant basis and structures on the tetrahedron" begin
    for n in (3, 6, 10)
        B = CR.invariant_basis(4, n)
        @test B.ranks == [CR.molien_coefficient(4, k) for k in 0:n]
        @test maximum(abs, B.Q' * B.Q - CR.LinearAlgebra.I) < 1e-12
    end
    ss = CR.candidate_structures(14, 5, 4)
    @test !isempty(ss)
    @test all(s -> CR.npoints(s) == 14, ss)
    @test all(s -> CR.nunknowns(s) >= CR.n_invariants(4, 5), ss)
    @test all(s -> count(o -> o.mult == [4], s.orbits) <= 1, ss)
    @test issorted(CR.nunknowns.(ss))
    # the triangle enumeration is unchanged by the generalisation
    @test [CR.nunknowns(s) for s in CR.candidate_structures(25, 10)] == [14, 15, 16, 17]
    # a tetrahedral moment-system Jacobian against finite differences
    s = CR.SymmetricStructure([[2, 1, 1], [3, 1], [3, 1], [3, 1]], 4)
    sys = CR.SymmetricMomentSystem(s, 6, Float64)
    θ = [0.004, 0.1, 0.2, 0.01, 0.05, 0.008, 0.15, 0.006, 0.3]
    r, J = sys(θ)
    h = 1e-7
    Jfd = similar(J)
    for j in eachindex(θ)
